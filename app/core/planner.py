...
这是阶段1/2 的规划器：只用等分 + 帧吸附，不做整体优化。
后续替换路径（按方案 §16 的阶段划分，别搞混归属）：
- **阶段3** 提供候选数据库（ASR 句末、语音间隙、镜头切换、黑场等真实候选点）；
- **阶段2** 在候选之上建候选图并用动态规划求解（§10.1–10.2），配 §5.3 剩余时长约束；
- **阶段4** 才是多模态剧情判断与边代价评分（§11）。

本模块届时保留为兜底路径与对照基线（§20.3 的"规则基线"）。
"""
